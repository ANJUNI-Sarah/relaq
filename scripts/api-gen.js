...
if (!process.env.API_URL) {
    console.error("❌ 錯誤：未設置 API_URL 環境變數");
    console.error("請在 .env 檔案中設置 API_URL");
    console.error("範例：API_URL=http://127.0.0.1:8000");
    process.exit(1);
}
...
${Object.entries(pathMapping)
    .map(
        ([operationId, { path }]) =>
            `    ${operationId.toUpperCase()}: \`\${process.env.API_URL}/api${path}\` as const,`
    )
    .join("\n")}
} as const;
